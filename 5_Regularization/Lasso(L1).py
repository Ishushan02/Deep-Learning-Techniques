...
def ridgeLoss(model, output, target, ridge_coeff):
    ...
    penalty = torch.sum(torch.abs(model.linear.weight))
...
def train_lasso(model, X, y, lambda_ridge, epochs=100, learning_rate=0.01):
...
loss_history = train_lasso(modelA, X, y, lambda_ridge = ridge_coeff)
loss_history = train_normal(modelB, X, y)

with torch.no_grad():
    lasso_predictions = modelA(X)
    normal_predictions = modelB(X)

plt.figure(figsize=(10,6))
print(X.shape, y.shape, lasso_predictions.numpy().shape)
plt.scatter(X.numpy(), y.numpy(), label='Data', color='blue')
plt.plot(X.numpy(), lasso_predictions.numpy(), label='Lasso Fit', color='red', linewidth=2)
